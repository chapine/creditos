        ...
        <div align="right" class="navegar_secciones">
            <div class="row" style="margin-left:0px;">
                <div class="btn-group btn-breadcrumb">
                    ...
                    <a href="" class="btn btn-success">Rendimiento global</a>
                </div>
            </div>
        </div>
        <!-- Navegador -->
    	...
    	<div class="panel panel-info">
			...
			<div class="panel-body">
				<div style="padding: 10px;">
					...
					<?php
						$sql2 = "SELECT count(cobrador) as cobrador FROM prestamos";
						$res2 = $mysqli->query($sql2);
						$row2 = $res2->fetch_assoc();
			
						$sql3 = "SELECT interes_pagar, fecha_inicial FROM prestamos";
						$res3 = $mysqli->query($sql3);
						//$row3 = $res3->fetch_assoc();
			
						$sql4 = "SELECT sum(interes_pagar) as interes FROM prestamos";
						$res4 = $mysqli->query($sql4);
						$row4 = $res4->fetch_assoc();
			
						$sql5 = "SELECT count(prestamo_activo) as activos FROM prestamos WHERE prestamo_activo = 1";
						$res5 = $mysqli->query($sql5);
						$row5 = $res5->fetch_assoc();
			
						$sql6 = "SELECT count(prestamo_activo) as finalizados FROM prestamos WHERE prestamo_activo = 0";
						$res6 = $mysqli->query($sql6);
						$row6 = $res6->fetch_assoc();
			
						$sql7 = "SELECT count(cobrador) as cobrador FROM clientes WHERE ocultar = 0";
						$res7 = $mysqli->query($sql7);
						$row7 = $res7->fetch_assoc();
			
						// TOTAL PRESTAMOS CON INTERES FINALIZADOS
						$sql8 = "SELECT sum(monto_prestado+interes_pagar) as monto_prestado_finalizado FROM prestamos WHERE prestamo_activo = 0";
						$res8 = $mysqli->query($sql8);
						$row8 = $res8->fetch_assoc();
			
						// TOTAL PRESTAMOS CON INTERES ACTIVOS
						$sql9 = "SELECT sum(monto_prestado+interes_pagar) as monto_prestado_activo FROM prestamos WHERE prestamo_activo = 1";
						$res9 = $mysqli->query($sql9);
						$row9 = $res9->fetch_assoc();
			
						// TOTAL COBROS FINALIZADOS
						$sql10 = "SELECT sum(total) as monto_cobrado_finalizado FROM detalleprestamo WHERE estado = 0";
						$res10 = $mysqli->query($sql10);
						$row10 = $res10->fetch_assoc();
			
						// TOTAL COBROS ACTIVOS
						$sql11 = "SELECT sum(total) as monto_cobrado_activo FROM detalleprestamo WHERE estado = 1";
						$res11 = $mysqli->query($sql11);
						$row11 = $res11->fetch_assoc();
			
						// TOTAL DE INTERESES
						$sql12 = "SELECT sum(interes_pagar) as intereses FROM prestamos";
						$res12 = $mysqli->query($sql12);
						$row12 = $res12->fetch_assoc();
			
						// TOTAL DE MORA
						$sql13 = "SELECT sum(mora) as mora FROM detalleprestamo";
						$res13 = $mysqli->query($sql13);
						$row13 = $res13->fetch_assoc();
			
						// PRESTAMOS ACTIVOS TOTAL
						$sql14 = "SELECT sum(monto_prestado) as monto_prestado FROM prestamos WHERE prestamo_activo = 1";
						$res14 = $mysqli->query($sql14);
						$row14 = $res14->fetch_assoc();
			
						// TOTAL PRESTAMOS SIN INTERES FINALIZADOS
						$sql15 = "SELECT sum(monto_prestado) as monto_prestado_finalizado FROM prestamos WHERE prestamo_activo = 0";
						$res15 = $mysqli->query($sql15);
						$row15 = $res15->fetch_assoc();
			
						// TOTAL PRESTAMOS SIN INTERES ACTIVOS
						$sql16 = "SELECT sum(monto_prestado) as monto_prestado_activo FROM prestamos WHERE prestamo_activo = 1";
					...
					<div class="row">
						<div class="col col-md-12 ajustar5">
							<table class="table table-striped table-bordered" width="100%" style="margin-bottom: 0px;">
								<tbody>
									<tr>
										<td width="30%" align="right"><b>Total de clientes</b></td>
							...
							<table class="table table-striped table-bordered" width="100%" style="margin-bottom: 0px;">
								<tbody>
									<tr>
										...
										<td width="70%">Q <?php echo moneda($row10['monto_cobrado_finalizado'], 'Q'); ?></td>
									</tr>
									<tr>
										<td align="right"><b>Monto cobrado activo (+)</b></td>
										<td style="border-bottom: 1px solid #9C0002;">Q <?php echo moneda($row11['monto_cobrado_activo'], 'Q'); ?></td>
									</tr>
									<tr>
										<td align="right"><b>Total cobrado (=)</b></td>
										<td style="border-bottom: 2px solid #9C0002;">Q <?php echo moneda(($row10['monto_cobrado_finalizado']+$row11['monto_cobrado_activo']), 'Q'); ?></td>
									</tr>
								</tbody>
							</table>
						</div>
