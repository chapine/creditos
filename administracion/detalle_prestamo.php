...
<head>
	<?php
		...
		if($u==0){
			...
			$alert_titulo 			="Error";
			$alert_mensaje 			="Antes de visualizar el prestamo debe de asiganar un cobrador a este cliente";
			$alert_icono 			="error";
			$alert_boton 			= "danger";
			$alert_link 			= "asignar_cobrador.php?id=$c";
		}
		...
		if(!empty($_POST)){
			...
			$fecha 				= mysqli_real_escape_string($mysqli,$_POST['fecha_pago']);
    ...
    <script>
		...
		{
			valor = document.getElementById("mora_recibida").value;
			if( valor == null || valor.length == 0 || /^\s+$/.test(valor) ) {
				swal({ title: "Si no desea cobrar mora ingrese el número cero (0)", type: "info", confirmButtonClass: "btn-info", confirmButtonText: "Aceptar", closeOnConfirm: false, },
					function(isConfirm){
						if(isConfirm){ swal.close(), registro.mora_recibida.focus(), no_enviar(); }
					}
				);
				return false;
			} else { return true;}
		...
		{
			valor = document.getElementById("fecha_pago").value;
			if( valor == null || valor.length == 0 || /^\s+$/.test(valor) ) {
				swal({ title: "Debe de seleccionar una fecha de pago", type: "info", confirmButtonClass: "btn-info", confirmButtonText: "Aceptar", closeOnConfirm: false, },
					function(isConfirm){
						if(isConfirm){ swal.close(), registro.fecha_pago.focus(), no_enviar(); }
					}
				);
				return false;
			} else { return true;}
		...
		function validar()
		{
			...
			if(validarCantidad() && validarInteres() && validarFechaPago())
			{
					...
					<div class="col col-md-12">
						...
						?>
							...
							<form id="registro" name="registro" action="<?php $_SERVER['PHP_SELF']."?a=b"; ?>" method="POST">
								<?php if($rowP['saldo_restante']=='0'){ $validar='onClick=""  disabled'; $disabled = 'disabled'; }else{ $validar='onClick="validar();"'; $disabled = ''; } ?>
								...
								<div class="col col-md-12">
									<div class="col-6 col-md-3 ajustar1">
										<label for="mora_recibida">Mora:</label>
									</div>
									<div class="col col-md-9 ajustar2">
										<div class="has-feedback">
											<input class="form-control" id="mora_recibida" name="mora_recibida" type="number" autocomplete="off" value="0" <?php echo $disabled; ?>>
								...
								<div class="col col-md-12">
									...
									<div class="col col-md-9 ajustar2">
										<div class="has-feedback">
											<input class="form-control" id="fecha_pago" name="fecha_pago" type="date" autocomplete="off" value="<?php echo fecha('Y-m-d', ''); ?>" <?php echo $disabled; ?>>
								...
								<div class="col col-md-12" align="right">
									...
									<div class="col col-md-9 ajustar3">
										...
										<button type="button" class="btn btn-success" id="regresar" onclick="location.href='ver_prestamos.php?id=<?php echo $c; ?>'" style="outline: none;">
											<span class="glyphicon glyphicon-backward"></span> Regresar
										</button>
									</div>
								</div>
							</form><!-- FORMULARIO PARA COBRAR  --  FORMULARIO PARA COBRAR  --  FORMULARIO PARA COBRAR -->
									...
									<table class="table table-striped table-bordered table-hover" width="100%">
										<tbody>
											<tr>
												...
												<td width="24%" align="center" style="font-weight: bold; color: #ffffff; background: #337ab7;">No de Boleta</td>
												<td class="hidden-xs" width="19%" align="center" style="font-weight: bold; color: #ffffff; background: #337ab7;">Fecha sugerida</td>
												<td class="hidden-xs" width="19%" align="center" style="font-weight: bold; color: #ffffff; background: #337ab7;">Fecha de pago</td>
											...
											<tr>
												...
												<td class="hidden-xs" align="center">
													<?php if($rowP['prestamo_activo']==0){ ?>
														<a href="" class="disabled"><i class="glyphicon glyphicon-pencil" style="font-size:14px;"></i></a>
													<?php }else{ ?>
														<a href="#" data-togglee="tooltip" data-placement="top" title="Editar" onclick="location.href='editar_pago.php?c=<?php echo $c; ?>&id=<?php echo $id; ?>&detalle=<?php echo $rowD[0]; ?>&cliente=<?php echo base64_encode($rowC['nombre']); ?>&abono_actual=<?php echo base64_encode($rowD[4]); ?>&mora_actual=<?php echo base64_encode($rowD[5]); ?>&ob=<?php echo base64_encode($rowD[10]); ?>'"><i class="glyphicon glyphicon-pencil"  style="font-size:14px;"></i></a>
													<?php } ?>
											...
											<?php
												if($_SESSION['id_usuario']==0 OR $_SESSION['id_usuario']==16){
											?>
												<td class="hidden-xs" align="center">
													<a href="javascript:void(0)" data-togglee="tooltip" data-placement="top" title="Eliminar" onclick="confirmar('<?php echo $c; ?>', '<?php echo $id; ?>', '<?php echo $rowD[0]; ?>', '<?php echo $rowD[4]; ?>', '<?php echo $rowD[5]; ?>', '<?php echo $rowP['prestamo_activo']; ?>', '<?php echo base64_encode($rowC['nombre']); ?>', '<?php echo base64_encode($rowP['monto_prestado']+$rowP['interes_pagar']); ?>');" style="color:red;"><i class="glyphicon glyphicon-trash"></i></a>
												</td>
											<?php }else{ ?>
												
												<td class="hidden-xs" align="center">
													<a href="javascript:void(0)" class="disabled" data-togglee="tooltip" data-placement="top" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>
												</td>
											<?php } ?>
