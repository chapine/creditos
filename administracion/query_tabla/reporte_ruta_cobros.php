	...
	if($estado == 1){
		$sql_contar = "SELECT count(*) AS numrows FROM detalleprestamo dp, prestamos p, clientes c, usuarios u, usuarios uu, rutas r, departamento d
					WHERE
						...
						(dp.id_clientes LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%' OR u.usuario LIKE '%".$query."%') ORDER BY dp.id_detalle ASC";
		...
		<tbody>
			...
			?>
				...
				<tr>
					...
					<td>
						<a href="#" data-toggle="tooltip" data-placement="top" title="Click para ver el perfil del cliente" onclick="location.href='perfil_cliente.php?id=<?php print($row['ID_CLIENTE']); ?>'">
					...
					<td>
						<a href="#" data-toggle="tooltip" data-placement="top" title="Click para ver el perfil del usuario" onclick="location.href='perfil_usuario.php?id=<?php print($row['ID_USUARIO']); ?>'">
							<i class="glyphicon glyphicon-link" style="color:#BBBBBB;"></i> <?php echo Convertir($row['USUARIO']); //Cobrador?>
						</a>
					</td>

					<td>
						<?php if($row['ID_USUARIO_REGISTRO']<1){ ?>
