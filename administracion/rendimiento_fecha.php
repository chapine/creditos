...
<body onload="Reloj()">
    ...
    <main id="page-wrapper6">
        ...
        <div align="right" class="navegar_secciones">
            <div class="row" style="margin-left:0px;">
                <div class="btn-group btn-breadcrumb">
                    ...
                    <a href="seleccionar_fecha.php?a=3" class="btn btn-success">Seleccionar fecha</a>
                    <a class="btn btn-danger">Rendimiento por fecha</a>
                </div>
            </div>
        </div>
        <!-- Navegador -->
        ...
        <div class="panel panel-info">
			<div class="panel-heading">
				<div class="btn-group pull-right">
					<a href="seleccionar_fecha.php?a=3" class="btn btn-success"><i class="glyphicon glyphicon-backward"></i> &nbsp; Regresar</a>
				</div>
    	...
    	<div class="panel panel-info">
			<div class="panel-heading">
				<h4><i class='glyphicon glyphicon-stats'></i> Estadística entre las fechas <?php echo fecha('d-m-Y', $fecha1).' - '.fecha('d-m-Y', $fecha2); ?></h4>
			</div>	
			...
			<div class="panel-body">
				<div style="padding: 10px;">
					...
					<?php
						$sql2 = "SELECT count(cobrador) as cobrador FROM prestamos WHERE cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						...
						$sql5 = "SELECT count(prestamo_activo) as activos FROM prestamos WHERE prestamo_activo = 1 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res5 = $mysqli->query($sql5);
						$row5 = $res5->fetch_assoc();
			
						$sql6 = "SELECT count(prestamo_activo) as finalizados FROM prestamos WHERE prestamo_activo = 0 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						...
						$sql8 = "SELECT sum(monto_prestado+interes_pagar) as monto_prestado_finalizado FROM prestamos WHERE prestamo_activo = 0 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res8 = $mysqli->query($sql8);
						$row8 = $res8->fetch_assoc();
			
						// TOTAL PRESTAMOS ACTIVOS
						$sql9 = "SELECT sum(monto_prestado+interes_pagar) as monto_prestado_activo FROM prestamos WHERE prestamo_activo = 1 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res9 = $mysqli->query($sql9);
						$row9 = $res9->fetch_assoc();
			
						// TOTAL COBROS FINALIZADOS
						$sql10 = "SELECT sum(total) as monto_cobrado_finalizado FROM detalleprestamo WHERE estado = 0 AND id_usuario = $id AND (fechaPago BETWEEN '$fecha1' AND '$fecha2')";
						$res10 = $mysqli->query($sql10);
						$row10 = $res10->fetch_assoc();
			
						// TOTAL COBROS ACTIVOS
						$sql11 = "SELECT sum(total) as monto_cobrado_activo FROM detalleprestamo WHERE estado = 1 AND id_usuario = $id AND (fechaPago BETWEEN '$fecha1' AND '$fecha2')";
						$res11 = $mysqli->query($sql11);
						$row11 = $res11->fetch_assoc();
			
						// TOTAL DE INTERESES
						$sql12 = "SELECT sum(interes_pagar) as intereses FROM prestamos WHERE cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res12 = $mysqli->query($sql12);
						$row12 = $res12->fetch_assoc();
			
						// TOTAL DE MORA
						$sql13 = "SELECT sum(mora) as mora FROM detalleprestamo WHERE id_usuario = $id AND (fechaPago BETWEEN '$fecha1' AND '$fecha2')";
						$res13 = $mysqli->query($sql13);
						$row13 = $res13->fetch_assoc();
			
						// PRESTAMOS ACTIVOS TOTAL
						$sql14 = "SELECT sum(monto_prestado) as monto_prestado FROM prestamos WHERE prestamo_activo = 1 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res14 = $mysqli->query($sql14);
						$row14 = $res14->fetch_assoc();
			
			
						// TOTAL PRESTAMOS SIN INTERES FINALIZADOS
						$sql15 = "SELECT sum(monto_prestado) as monto_prestado_finalizado FROM prestamos WHERE prestamo_activo = 0 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
						$res15 = $mysqli->query($sql15);
						$row15 = $res15->fetch_assoc();
			
						// TOTAL PRESTAMOS SIN INTERES ACTIVOS
						$sql16 = "SELECT sum(monto_prestado) as monto_prestado_activo FROM prestamos WHERE prestamo_activo = 1 AND cobrador = $id AND (fecha_inicial BETWEEN '$fecha1' AND '$fecha2')";
							...
							<table class="table table-striped table-bordered" width="100%" style="margin-bottom: 0px;">
								<tbody>
									<tr>
										...
										<td width="70%"<?php echo moneda($row8['monto_prestado_finalizado'], 'Q'); ?></td>
									</tr>
									<tr>
							...
							<table class="table table-striped table-bordered" width="100%" style="margin-bottom: 0px;">
								<tbody>
									<tr>
										...
										<td width="70%"><?php echo moneda($row12['intereses'], 'Q'); ?></td>
									</tr>
									<tr>
										<td align="right"><b>Total de Mora</b></td>
										<td><?php echo moneda($row13['mora'], 'Q'); ?></td>
									</tr>
								</tbody>
							</table>
						</div>
