<?php
	...
	if($action == 'ajax'){
		...
		if($opt == 1){ // MOSTRAR TODOS LOS CLIENTES
			$a = 1;
			include('../administracion/query_tabla/lista_clientes.php');
		}
		...
		if($opt == 3){  // MOSTRAR CLIENTES SIN PRESTAMOS
			...
			include('../administracion/query_tabla/lista_clientes_prestamos.php');
		}
		...
		if($opt == 4){  // MOSTRAR PRESTAMOS
			...
			include('../administracion/query_tabla/lista_prestamos.php');
		}
		...
		if($opt == 5){  // MOSTRAR COBROS REALIZADOS POR FECHAS
			...
			include('../administracion/query_tabla/cobros_realizados.php');
		}
		...
		if($opt == 6){  // LISTA DE RUTAS
			...
			include('../administracion/query_tabla/reporte_ruta.php');
		}
		
		if($opt == 7){  // LISTA DE RUTAS
			$a = 1;
			include('../administracion/query_tabla/lista_rutas.php');
		}
		...
		if($opt == 8){  // COBROS PARA HOY
			...
			include('../administracion/query_tabla/cobros_hoy.php');
		}
		...
		if($opt == 9){  // LISTA DE RUTAS
			...
			include('../administracion/query_tabla/reporte_ruta_cobros.php');
		}
		
		
		if($opt == 11){ // LISTA DE USUARIOS
			$a = 1;
			if($_SESSION['tipo_usuario']=='administracion'){ include('../administracion/query_tabla/lista_usuarios_s.php'); }
		}
		
		if($opt == 12){ // LISTA DE COBRADORES
			$a = 1;
			include('../administracion/query_tabla/lista_cobradores.php');
		}
		...
		if($opt == 13){ // BITACORA DEL CAPITAL :V
			...
			include('../administracion/query_tabla/bitacora.php');
		}
		...
		if($opt == 14){ // PRESTAMOS VENCIDOS
			...
			include('../administracion/query_tabla/lista_prestamos_vencidos.php');
		}
		...
		if($opt == 15){ // PRESTAMOS VENCIDOS
			...
			include('../administracion/query_tabla/lista_prestamos_renovados.php');
		}
		
		if($opt == 16){ // LISTA DE SESIONES ACTIVAS
			$a = 1;
			include('../administracion/query_tabla/sesiones.php');
		}
		...
		if($opt == 17){  // LISTA DE RUTAS
			...
			include('../administracion/query_tabla/reporte_ruta_renovaciones.php');
		}
		...
		if($opt == 18){  // LISTA DE RUTAS
			...
			include('../administracion/query_tabla/reporte_ruta_renovaciones_cobros.php');
		}
		...
		?>
			...
			<div id="scroll_x" style="overflow-x: auto; overflow-y: auto; white-space:nowrap">
				<?php
					...
					if($opt == 11){ // LISTA DE USUARIOS
						$a = 2;
						//include('../administracion/query_tabla/lista_usuarios.php');
						if($_SESSION['tipo_usuario']=='administracion'){ include('../administracion/query_tabla/lista_usuarios_s.php'); }
					}
