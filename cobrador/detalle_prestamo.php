...
<head>
	<?php
		...
		$sqlC = "SELECT * FROM clientes WHERE id_cliente = $c AND cobrador = $usuario";
		$resC = $mysqli->query($sqlC);
		$rowC = $resC->fetch_assoc();
		
		$u = $rowC['cobrador'];
		
		if($rowC == 0){header ("Location: ver_prestamos.php?id=$c");}
			
		$sqlU = "SELECT * FROM usuarios WHERE id_usuario = ".$rowC['cobrador'];
		...
		if(!empty($_POST)){
			...
			$mora_recibida 		= mysqli_real_escape_string($mysqli, $_POST['mora_recibidaa']);
			...
			if( $no_boleta == '' ){
				...
				if($cantidad_recibida > $saldo_restante){
					$alert_bandera = true;
					$alert_titulo ="Error";
					...
					$sqlPago = "INSERT INTO detalleprestamo (id_detalle, id_clientes, id_prestamo, id_usuario, abono, mora, total, fecha_sugerida, fechaPago, estado, observaciones, usuario)
					VALUES
					(NULL, '$c', '$id', '$u', '$cantidad_recibida', '$mora_recibida', '$total', '$fecha_sugerida', '$fecha', '1', '$observaciones', '$usuario');";
					...
					if($saldo_restante == $cantidad_recibida){ //TERMINO DE PAGAR
						$sqlPrestamo = "UPDATE prestamos SET fecha_siguiente='1000-01-01', saldo_restante='0.00', prestamo_activo='0' WHERE id_prestamo=$id";
					...
					if($cantidad_recibida < $saldo_restante){ //AUN NO TERMINA DE PAGAR
						...
						$sqlPrestamo = "UPDATE prestamos SET fecha_siguiente='$FechaSig', saldo_restante='$nuevo_saldo_restante', prestamo_activo='1', fecha_pago_diario='$fecha_pago_diario' WHERE id_prestamo=".$id;
						$resPrestamo = $mysqli->query($sqlPrestamo);
					}

					if($resPago > 0 AND $resPrestamo > 0){
						/*$alert_bandera = true;*/
				...
				if( $boleta_ > 0 ){
					...
					if($cantidad_recibida > $saldo_restante){
						...
						$sqlPago = "INSERT INTO detalleprestamo (id_detalle, id_clientes, id_prestamo, id_usuario, abono, mora, total, fecha_sugerida, fechaPago, estado, observaciones, usuario, boleta_deposito)
						VALUES
						(NULL, '$c', '$id', '$u', '$cantidad_recibida', '$mora_recibida', '$total', '$fecha_sugerida', '$fecha', '1', '$observaciones', '$usuario', '$no_boleta');";
						$resPago = $mysqli->query($sqlPago);
...
<body onload="Reloj()">
			...
			<div class="panel-body">
				...
				<div class="row">
					<div class="col col-md-12" style=" padding-bottom: 5px; text-align: center; font-size: 18px;">
						<b>Datos personales</b> <br><br><div style="border:1px solid #ccc;"></div><br>
					</div>
					...
					<div class="col col-md-12" style="padding-bottom: 5px; text-align: center;">
						<?php
							...
							if($contar_a == 0 AND $contar_b == 0){ // IF para comprobar si no hay registros
								echo '<div class="alert alert-info" role="alert" style="text-align: center;"><b>Este préstamo no tiene renovaciones</b></div>';
					...
					<!-- Cobrar -->			
					<?php if($rowP['prestamo_activo']<>0){ // <-- COMPROBAMOS SI EL PRESTAMO ESTA ACTIVO --> ?>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tbody>
												<tr>
													...
													<td valign="middle">
														<b>
															El cobro del día ya fue realizado.<br>Sí desea ingresar un pago con fecha anterior contacte a un administrador.
														</b>
													</td>
												</tr>
											</tbody>
									...
									<table class="table table-striped table-bordered table-hover" width="100%">
										<tbody>
											<tr>
												...
												<td width="18%" align="center" style="font-weight: bold; color: #ffffff; background: #337ab7;">No de Boleta</td>
											...
											<tr>
												...
												<td class="hidden-xs" align="center"><?php if($rowD[7]=='1000-01-01'){echo '<font style="color: #8B8B8B;">0000-00-00</font>';}else{ echo fecha('d-m-Y', $rowD[7]);} ?></td>
    ...
    <?php
		...
		if($_SESSION['mensaje'] <> ""){
			...
			$mensaje_error = $_SESSION['mensaje'];
			$color_mensaje = $_SESSION['color_mensaje'];
			$icono_mensaje = $_SESSION['icono_mensaje'];
