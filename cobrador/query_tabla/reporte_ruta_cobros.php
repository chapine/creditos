<?php
	if($estado == 1){
		$sql_contar = "SELECT count(*) AS numrows FROM detalleprestamo dp, clientes c, usuarios u, usuarios uu, rutas r, departamento d
					WHERE
						c.id_cliente = dp.id_clientes AND u.id_usuario = dp.id_usuario AND r.id_ruta = c.id_ruta AND d.id_departamento = r.id_departamento AND
	...
	if($estado == 2){
		$sql_contar = "SELECT count(*) AS numrows FROM detalleprestamo dp, clientes c, usuarios u, usuarios uu, rutas r, departamento d
				WHERE c.id_cliente = dp.id_clientes AND u.id_usuario = dp.id_usuario AND r.id_ruta = c.id_ruta AND
                        d.id_departamento = r.id_departamento AND c.id_ruta = '$ruta' AND dp.id_usuario = '$cobrador' AND (dp.fechaPago BETWEEN '$fecha1' AND '$fecha2') AND uu.id_usuario = dp.usuario AND
						(dp.id_clientes LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%' OR u.nombre LIKE '%".$query."%')";
	...
	if($estado == 3){
		$sql_contar = "SELECT count(*) AS numrows FROM detalleprestamo dp, clientes c, usuarios u, usuarios uu, rutas r, departamento d
				WHERE c.id_cliente = dp.id_clientes AND u.id_usuario = dp.id_usuario AND r.id_ruta = c.id_ruta AND
                        d.id_departamento = r.id_departamento AND dp.id_usuario = '$cobrador' AND (dp.fechaPago BETWEEN '$fecha1' AND '$fecha2') AND uu.id_usuario = dp.usuario AND
						(dp.id_clientes LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%' OR u.nombre LIKE '%".$query."%')";
	...
	if($estado == 4){
		$sql_contar = "SELECT count(*) AS numrows FROM detalleprestamo dp, clientes c, usuarios u, usuarios uu, rutas r, departamento d
				WHERE c.id_cliente = dp.id_clientes AND u.id_usuario = dp.id_usuario AND r.id_ruta = c.id_ruta AND
                        d.id_departamento = r.id_departamento AND c.id_ruta = '$ruta' AND (dp.fechaPago BETWEEN '$fecha1' AND '$fecha2') AND uu.id_usuario = dp.usuario AND
						(dp.id_clientes LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%' OR u.nombre LIKE '%".$query."%')";
					...
					<td>
						<?php if($row['ID_USUARIO_REGISTRO']<=0){ ?>
							<div style="color:#BBBBBB;">
								<i class="glyphicon glyphicon-link"></i> <?php echo Convertir($row['USUARIO_REGISTRO']); //Cobrador?>
							</div>
						<?php }else{ ?>

							<a href="#" data-toggle="tooltip" data-placement="top" title="Click para ver el perfil del usuario" onclick="location.href='perfil_usuario.php?id=<?php print($row['ID_USUARIO_REGISTRO']); ?>'" style="color: black; text-decoration: none;">
