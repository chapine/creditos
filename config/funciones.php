<?php
	...
	function Convertir($sentence){
		$cadena = mb_convert_case($sentence, MB_CASE_TITLE, "UTF-8");
	...
	function verificar($cadena){
		if($cadena=='' OR $cadena=='Ninguno'){
			$cadena = '<span style="color: #cccccc;">Ninguno</span>';
		}else{
	...
	function telefono($cadena, $input){
		if($input=='input'){
			...
			if($cadena==''){
				$cadena = '<i class="glyphicon glyphicon-phone-alt"></i> 0000-0000';
			}else{
				$cadena = intval(preg_replace('/[^0-9]+/', '', $cadena), 10); 
				$cadena = '<i class="glyphicon glyphicon-phone-alt"></i> '.substr($cadena, -8, -4)."-".substr($cadena, -4);
			}
		}
	...
	function celular($cadena, $input){
		if($input=='input'){
			...
			if($cadena==''){
				$cadena = '<i class="glyphicon glyphicon-phone"></i> 0000-0000';
			}else{
				$cadena = intval(preg_replace('/[^0-9]+/', '', $cadena), 10); 
				$cadena = '<i class="glyphicon glyphicon-phone"></i> '.substr($cadena, -8, -4)."-".substr($cadena, -4);
			}
		}
	...
	function dpi($cadena, $input){
		if($input=='input'){
			...
			if($cadena==''){
				...
				$cadena = intval(preg_replace('/[^0-9]+/', '', $cadena), 10); 
				$cadena = '<i class="glyphicon glyphicon-credit-card"></i> '.substr($cadena,-14,-9)."-".substr($cadena,-9,-4)."-".substr($cadena,-4);
			}
		}
	...
	function ruta($cadena){
		...
		if($cadena=='Ninguno, Ninguno'){
			$cadena = '<i class="glyphicon glyphicon-road"></i> Ninguno, Ninguno';
		}else{
			$cadena = '<i class="glyphicon glyphicon-road"></i> '.Convertir($cadena);
		}
