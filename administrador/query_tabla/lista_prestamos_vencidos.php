<?php
	if($estado == 1){
		$sql_contar = "SELECT count(*) AS numrows FROM prestamos p, clientes c, usuarios u, rutas r, departamento d WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' AND r.id_ruta = p.id_ruta AND d.id_departamento = r.id_departamento AND (c.id_cliente LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR u.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%')";
		...
		$sql_query = "SELECT p.id_prestamo, c.nombre, u.nombre, p.mensualidades, p.monto_prestado, p.fecha_inicial, p.interes_pagar, p.cuota, p.saldo_restante, p.prestamo_activo, p.id_clientes, p.fecha_siguiente, p.cobrador, p.id_ruta, p.fecha_final, c.id_cliente, concat_ws(', ', r.nombre, d.nombre) AS RUTA
			...
			WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' AND r.id_ruta = p.id_ruta AND d.id_departamento = r.id_departamento AND
	...
	if($estado == 2){
		$sql_contar = "SELECT count(*) AS numrows FROM prestamos p, clientes c, usuarios u, rutas r, departamento d WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' AND p.cobrador = '$usuario' AND r.id_ruta = p.id_ruta AND d.id_departamento = r.id_departamento AND (c.id_cliente LIKE '%".$query."%' OR c.nombre LIKE '%".$query."%' OR u.nombre LIKE '%".$query."%' OR r.nombre LIKE '%".$query."%' OR d.nombre LIKE '%".$query."%')";
		
		$sql_query = "SELECT p.id_prestamo, c.nombre, u.nombre, p.mensualidades, p.monto_prestado, p.fecha_inicial, p.interes_pagar, p.cuota, p.saldo_restante, p.prestamo_activo, p.id_clientes, p.fecha_siguiente, p.cobrador, p.id_ruta, p.fecha_final, c.id_cliente, concat_ws(', ', r.nombre, d.nombre) AS RUTA
			
			FROM prestamos p, clientes c, usuarios u, rutas r, departamento d
			
			WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' AND p.cobrador = '$usuario' AND r.id_ruta = p.id_ruta AND d.id_departamento = r.id_departamento AND
			...
			?>
				<tr>
					...
					<td align="center">
						<b>
						<?php
							$fecha_inicial = fecha('d-m-Y', $row[14]);
							$fecha_final   = fecha('d-m-Y', $fecha);
					...
					<td align="center">
						<div class="btn-group">
							<button type="button" class="btn btn-xs btn-xs btn-success" onclick="location.href='renovar.php?c=<?php echo $row[10]; ?>&p=<?php echo $row[0]; ?>'">
								<i class="glyphicon glyphicon-ok"></i> Renovar
							</button>
						</div>
					</td>
				</tr>
