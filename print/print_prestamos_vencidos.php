...
<head>
	...
	<?php
		...
		if($a==1){
			...
			WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' order by p.id_prestamo";
		...
		if($a==2){
			...
			WHERE c.id_cliente = p.id_clientes AND u.id_usuario = p.cobrador AND p.prestamo_activo = 1 AND p.fecha_final < '$fecha' AND p.cobrador = '$u' order by p.id_prestamo";
		...
		<tbody>
			...
			?>
				<tr>
					...
					<td align="center">
						<?php
							echo '<span class="label label-warning" data-toggle="tooltip" data-placement="left" title="Atrasado" style="font-size: 11px; cursor:pointer;"><font color="#000000">'.SumarDia($row[14], $fecha).' Dia(s)</font></spam>';
						?>
					</td>
				</tr>
